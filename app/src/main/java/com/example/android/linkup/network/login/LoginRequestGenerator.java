...
public class LoginRequestGenerator {
    ...
    public static Request generate (Command onSuccessCommand, Command onErrorCommand, Command hideProgressBar, Context c, Photos photos) {
        String url = NetworkConfiguration.getInstance().serverAddr;
        url += PROFILE_ENDPOINT;
        JSONObject obj = new JSONObject();
        LoginErrorListener errorListener = new LoginErrorListener(onErrorCommand, hideProgressBar,c);
        LoginResponseListener responseListener = new LoginResponseListener(onSuccessCommand, hideProgressBar, photos);
